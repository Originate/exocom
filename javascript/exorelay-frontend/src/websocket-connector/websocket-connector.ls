...
class WebSocketConnector extends EventEmitter
  ...
  close: ->
    ...
    debug "no longer connected at 'ws://#{@host}/#{@port}'"
    @socket?.close!
    @emit 'offline'
  ...
  send: (message-name, payload, options = {}) ->
    | !message-name                      =>  return @emit 'error', new Error 'ExoRelayFrontend#send cannot send empty messages'
    | typeof message-name isnt 'string'  =>  return @emit 'error', new Error 'ExoRelayFrontend#send can only send string messages'
    | typeof payload is 'function'       =>  return @emit 'error', new Error 'ExoRelayFrontend#send cannot send functions as payload'
  ...
  _on-socket-error: (error) ~>
    | error.errno is 'EADDRINUSE'   =>  @emit 'error', "port #{@port} is already in use"
